...
import (
	...
	"github.com/rodbate/jvm-on-go/native/constants"
...
func checkArrayStore(src, dest *rtda.Object) bool {
	srcClass := src.Class()
...
func initProperties(frame *rtda.Frame) {
	...
	for k, v := range systemProperties {
		key := rtda.GetJString(classloader, k)
		value := rtda.GetJString(classloader, v)

		stack := rtda.NewOperandStack(3)
...
func mapLibraryName(frame *rtda.Frame) {
	...
	*/
	...
	libNameGo := rtda.GetGoString(libName)
	frame.OperandStack.PushRef(rtda.GetJString(frame.Method().Class().ClassLoader(), buildLibraryName(libNameGo)))
}
