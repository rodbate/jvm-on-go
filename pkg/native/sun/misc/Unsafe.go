...
func arrayBaseOffset(frame *rtda2.Frame) {
	frame.OperandStack.PushInt(0)
}

//public native int arrayIndexScale(Class<?> arrayClass);
func arrayIndexScale(frame *rtda2.Frame) {
	frame.OperandStack.PushInt(1)
}

//public native int addressSize();
func addressSize(frame *rtda2.Frame) {
	addressSize := unsafe.Sizeof(frame.LocalVars.GetThis())
	frame.OperandStack.PushInt(int32(addressSize))
}

//public native long objectFieldOffset(Field f);
func objectFieldOffset(frame *rtda2.Frame) {
...
public final native boolean compareAndSwapObject(Object o, long offset,
												 ...
												 Object x);
*/
func compareAndSwapObject(frame *rtda2.Frame) {
	...
	if data, ok := objData.(rtda2.Slots); ok {
		//object
	...
	} else if data, ok := objData.(rtda2.ArrayRef); ok {
		//array
		val := data[offset]
		var cas bool
		if val == expectedVal {
...
func getIntVolatile(frame *rtda2.Frame) {
	obj := frame.LocalVars.GetRef(1)
	offset := frame.LocalVars.GetLong(2)
	if data, ok := obj.Data.(rtda2.Slots); ok {
		val := data.GetInt(uint(offset))
		frame.OperandStack.PushInt(val)
	} else if data, ok := obj.Data.(rtda2.ArrayInt); ok {
...
public final native boolean compareAndSwapInt(Object o, long offset,
											  ...
											  int x);
*/
func compareAndSwapInt(frame *rtda2.Frame) {
	objData := frame.LocalVars.GetRef(1).Data
	offset := frame.LocalVars.GetLong(2)
	...
	if data, ok := objData.(rtda2.Slots); ok {
		...
		frame.OperandStack.PushBoolean(cas)
	} else if data, ok := objData.(rtda2.ArrayInt); ok {
		val := data[offset]
		var cas bool
		if val == expectedVal {
			data[offset] = newVal
...
*/
func compareAndSwapLong(frame *rtda2.Frame) {
	objData := frame.LocalVars.GetRef(1).Data
	offset := frame.LocalVars.GetLong(2)
	...
	if data, ok := objData.(rtda2.Slots); ok {
		...
		frame.OperandStack.PushBoolean(cas)
	} else if data, ok := objData.(rtda2.ArrayLong); ok {
		val := data[offset]
		var cas bool
		if val == expectedVal {
			data[offset] = newVal
...
func getByte(frame *rtda2.Frame) {
	address := frame.LocalVars.GetLong(1)
	mem := memoryAt(address)
...
func freeMemory(frame *rtda2.Frame) {
	address := frame.LocalVars.GetLong(1)
	free(address)
}

//public native Object getObjectVolatile(Object o, long offset);
func getObjectVolatile(frame *rtda2.Frame) {
	obj := frame.LocalVars.GetRef(1)
	offset := frame.LocalVars.GetLong(2)
	if data, ok := obj.Data.(rtda2.Slots); ok {
		val := data.GetRef(uint(offset))
		frame.OperandStack.PushRef(val)
	} else if data, ok := obj.Data.(rtda2.ArrayRef); ok {
